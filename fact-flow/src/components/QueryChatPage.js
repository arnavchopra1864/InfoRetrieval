...
function QueryChatPage() {
  ...
  const handleDocSel = (docNum) => {
    ...
    const hostname =
      "https://factflow-backend-39dce4f4a0a2.herokuapp.com/search?u=139801c9-88f7-4cbf-829e-135feba4df6e&d=";
  ...
  const handleSubmit = () => {
    const hostname =
      "https://factflow-backend-39dce4f4a0a2.herokuapp.com/search/query?";
    var query_link = "";
    if (inputValue.trim() === "") {
      ...
      query_link =
        hostname +
        "u=" +
        "139801c9-88f7-4cbf-829e-135feba4df6e" +
        "&d=" +
        docid +
        "&q=" +
  ...
  return (
    <>
      <NavBar activeName={"Query"} />
      <div>
        x<h1>Chat with a document</h1>
        <div>
          <div>
            <div>
              {Array.from({ length: 5 }, (_, index) => (
                <button onClick={handleDocSel(index + 1)} key={index + 1}>
                  {index + 1}
                </button>
              ))}
            </div>
        ...
        <div className="doctext" id="document">
          {" "}
          doc goes here
        </div>
      </div>
